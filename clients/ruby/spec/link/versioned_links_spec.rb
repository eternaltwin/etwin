...
RSpec.describe Etwin::Link::VersionedLinks do # rubocop:disable Metrics/BlockLength
  items = SerializationTestItem.from_test_dir(
    'link/versioned-links',
    {
      'demurgos' => Etwin::Link::VersionedLinks.new(
  ...
  items.each do |item|
    it "reads #{item.name}" do
      actual = Etwin::Link::VersionedLinks.from_json(item.json)
