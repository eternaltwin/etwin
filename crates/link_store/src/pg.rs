...
use etwin_core::hammerfest::{HammerfestStore, HammerfestUserId, GetHammerfestUserOptions, ShortHammerfestUser, HammerfestServer, HammerfestUsername, ArchivedHammerfestUser, HammerfestUserIdRef};
use std::error::Error;
use std::ops::Deref;
use sqlx;
use sqlx::PgPool;
use etwin_core::link::{LinkStore, TouchLinkError, GetLinkOptions, TouchLinkOptions, VersionedRawLink, RawLink};
      ...
      Some(row) => {
        let link: VersionedRawLink<HammerfestUserIdRef> = VersionedRawLink {
          current: Some(RawLink {
            link: RawUserDot { time: row.linked_at, user: UserIdRef { id: row.linked_by } },
            unlink: (),
